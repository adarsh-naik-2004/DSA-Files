...
public:
    ...
    int recur(vector<int>& prices, int i, int buy){
        if(i>=prices.size()){
            return 0;
        }
        ...
        if(buy){
            int profit1 = recur(prices,i+1,0) - prices[i];
            int ignore1 = recur(prices,i+1,1);
            maxi = max(maxi,max(profit1,ignore1));
        }
        else{
            int profit2 = recur(prices,i+1,1) + prices[i];
            int ignore2 = recur(prices,i+1,0);
    ...
    int memo(vector<int>& prices, int i, int buy, vector<vector<int>>& dp){
        ...
        if(buy){
            ...
            int profit2 = memo(prices,i+1,1,dp) + prices[i];
